...
function unsaveApartment(place_id) {
  let savedApartments = getSavedApartments();
  savedApartments = savedApartments.filter(a => a.place_id !== place_id);
  saveApartmentsToStorage(savedApartments);
...
window.displaySavedApartments = function() {
  ...
  savedApartments.forEach(apartment => {
    ...
    detailsDiv.className = 'saved-apartment-item-details';
    detailsDiv.innerHTML = `
      <strong>${apartment.name}</strong>
      <span>${apartment.vicinity || 'Address not available'}</span>
      ${apartment.website ? `<a href="${apartment.website}" target="_blank">Visit Website</a>` : ''}
    `;

    const unsaveBtn = document.createElement('button');
    unsaveBtn.innerText = 'Unsave';
    unsaveBtn.className = 'unsave-button';
    unsaveBtn.addEventListener('click', () => {
      unsaveApartment(apartment.place_id);
    });

    li.appendChild(photoHtml ? createElementFromHTML(photoHtml) : document.createTextNode(''));
...
// Helper to convert HTML string to Node
